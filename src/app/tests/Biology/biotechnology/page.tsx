"use client";

import Link from "next/link";

export default function BiotechnologyPage() {
  const tests = [
    {
      title: "Biotechnology — Test 1",
      questions: 20,
      time: 20,
      difficulty: "Basic",
    },
    {
      title: "Biotechnology — Test 2",
      questions: 35,
      time: 35,
      difficulty: "Moderate",
    },
    {
      title: "Biotechnology — Test 3",
      questions: 50,
      time: 50,
      difficulty: "MDCAT",
    },
    {
      title: "Biotechnology — Test 4",
      questions: 50,
      time: 50,
      difficulty: "MDCAT",
    },
    {
      title: "Biotechnology — Test 5",
      questions: 50,
      time: 50,
      difficulty: "MDCAT",
    },
  ];

  return (
    <main className="min-h-screen bg-[#0b1e39] text-white">
      {/* HEADER */}
      <header className="border-b border-[#172d4f]">
        <div className="container mx-auto px-4 py-5 flex items-center justify-between">
          <Link
            href="/"
            className="text-2xl font-black tracking-tight"
          >
            STUDYING{" "}
            <span className="text-[#ff9800]">TACTICS</span>
          </Link>

          <Link
            href="/tests/Biology"
            className="text-sm font-semibold text-[#cdd6e6] hover:text-[#ff9800] transition"
          >
            ← Back to Biology
          </Link>
        </div>
      </header>

      {/* PAGE CONTENT */}
      <section className="container mx-auto px-4 py-12">
        {/* TITLE */}
        <div className="text-center">
          <p className="text-[#ff9800] font-semibold uppercase tracking-wider text-sm">
            Biology
          </p>

          <h1 className="text-4xl md:text-5xl font-black mt-2">
            Biotechnology
          </h1>

          <p className="text-[#cdd6e6] max-w-2xl mx-auto mt-4">
            Choose a test and challenge yourself with MDCAT-style
            questions.
          </p>
        </div>

        {/* TEST LIST */}
        <div className="max-w-4xl mx-auto mt-12 space-y-5">
          {tests.map((test, index) => (
            <div
              key={test.title}
              className="bg-white text-[#0b1e39] rounded-2xl p-6 md:p-7 shadow-lg flex flex-col md:flex-row md:items-center md:justify-between gap-5"
            >
              {/* TEST INFORMATION */}
              <div>
                <div className="flex items-center gap-3 flex-wrap">
                  <span className="bg-[#e9ecef] rounded-full px-3 py-1 text-xs font-bold">
                    TEST {index + 1}
                  </span>

                  <span className="bg-[#fff3e0] text-[#e38000] rounded-full px-3 py-1 text-xs font-bold">
                    {test.difficulty}
                  </span>

                  <span className="bg-green-100 text-green-700 rounded-full px-3 py-1 text-xs font-bold">
                    AVAILABLE
                  </span>
                </div>

                <h2 className="text-xl md:text-2xl font-bold mt-3">
                  {test.title}
                </h2>

                <div className="flex flex-wrap gap-5 text-sm text-zinc-600 mt-2">
                  <span>
                    📝 {test.questions} Questions
                  </span>

                  <span>
                    ⏱️ {test.time} Minutes
                  </span>
                </div>
              </div>

              {/* ACTION BUTTON */}
              <Link
                href={`/tests/Biology/biotechnology/test-${index + 1}`}
                className="bg-[#ff9800] hover:bg-[#e68900] text-white font-bold px-7 py-3 rounded-xl whitespace-nowrap transition text-center"
              >
                Start Test →
              </Link>
            </div>
          ))}
        </div>

        {/* INFORMATION CARD */}
        <div className="max-w-4xl mx-auto mt-10 bg-[#142542] border border-[#243b60] rounded-2xl p-6">
          <h3 className="text-lg font-bold text-[#ff9800]">
            Before you start
          </h3>

          <ul className="mt-3 space-y-2 text-sm text-[#cdd6e6]">
            <li>• Each question has one correct answer.</li>
            <li>• The timer will start when you begin the test.</li>
            <li>• Your score will be calculated automatically.</li>
            <li>
              • You will be able to review your answers after submission.
            </li>
          </ul>
        </div>
      </section>
    </main>
  );
}