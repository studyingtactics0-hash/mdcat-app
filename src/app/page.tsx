...
const defaultSubjectStats: Record<string, SubjectStats> = {
  Biology: {
    tests: 0,
    questions: 0,
    correct: 0,
...
export default function Home() {
  ...
  const [subjectStats, setSubjectStats] =
    useState<Record<string, SubjectStats>>(defaultSubjectStats);
  ...
  useEffect(() => {
    async function getStatistics(userId: string) {
      const { data, error } = await supabase
        .from("test_results")
        .select(
          "total_questions, correct_answers, score_percentage, completed_at, subject"
        )
        .eq("user_id", userId)
      ...
      if (!data || data.length === 0) {
        ...
        setSubjectStats(defaultSubjectStats);
        return;
      }
      ...
      const calculatedSubjectStats: Record<string, SubjectStats> = {
        Biology: {
          ...defaultSubjectStats.Biology,
        },
        Chemistry: {
          ...defaultSubjectStats.Chemistry,
        },
        Physics: {
          ...defaultSubjectStats.Physics,
        },
        English: {
          ...defaultSubjectStats.English,
        },
        "Logical Reasoning": {
          ...defaultSubjectStats["Logical Reasoning"],
        },
      };
      ...
      Object.keys(calculatedSubjectStats).forEach((subject) => {
        const subjectData = calculatedSubjectStats[subject];

        if (subjectData.tests > 0) {
          subjectData.averageScore = Math.round(
            subjectData.averageScore / subjectData.tests
          );
        } else {
        ...
        if (subjectData.questions > 0) {
          subjectData.accuracy = Math.round(
  ...
  const strongestSubject =
    attemptedSubjects.length > 0
      ? attemptedSubjects.reduce((best, current) =>
        ...
        <section className="container mx-auto flex flex-col-reverse md:flex-row items-center justify-between px-4 py-12 md:py-20 gap-10">
          ...
          <div className="flex-1 flex flex-col md:items-start items-center">
            ...
            <p className="text-[#cdd6e6] text-base md:text-lg mb-8 text-center md:text-left max-w-xl">
              Practice with high-quality MDCAT tests, track your performance,
              identify your weaknesses and stay updated with everything that
              matters.
            </p>
        ...
        <section className="container mx-auto px-4 py-10 md:py-14">
          ...
          <div className="max-w-lg mx-auto bg-white rounded-2xl shadow-lg p-7 border-l-4 border-[#ff9800]">
            ...
            <p className="text-zinc-600 text-sm mb-5">
              Test your preparation with full-length mock tests covering the
              MDCAT syllabus.
            </p>
        ...
        <section className="container mx-auto px-4 py-10 md:py-14">
          ...
          <div className="w-full max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-5">
            <div className="bg-white rounded-2xl shadow p-6 flex flex-col items-center">
              ...
              <span className="text-[#576a89] text-sm text-center">
                Tests Attempted
              </span>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-xl p-7 border-l-4 border-[#ff9800]">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  {strongestSubject ? (
                    <>
                      <h3 className="text-2xl font-black text-[#0b1e39]">
                        {subjectIcons[strongestSubject[0]]}{" "}
                        {strongestSubject[0]}
                      </h3>

                      <p className="text-sm text-[#576a89] mt-1">
                        {strongestSubject[1].accuracy}% accuracy
                      </p>
                    </>
                  ) : (
                    <h3 className="text-xl font-bold text-[#0b1e39]">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  {weakestSubject ? (
                    <>
                      <h3 className="text-2xl font-black text-[#0b1e39]">
                        {subjectIcons[weakestSubject[0]]}{" "}
                        {weakestSubject[0]}
                      </h3>

                      <p className="text-sm text-[#576a89] mt-1">
                        {weakestSubject[1].accuracy}% accuracy
                      </p>
                    </>
                  ) : (
                    <h3 className="text-xl font-bold text-[#0b1e39]">
